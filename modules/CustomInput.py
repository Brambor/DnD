...
class CustomInput():
	...
	def write_to_log(self, message, res):
		...
		if not os.path.exists(logs_path):
			os.mkdir(logs_path)
		with open(f"{logs_path}/{self.C.log_file}.txt", "a", encoding="utf-8") as log_file:
			log_file.write(f"{message} {res}\n")
		with open(f"{logs_path}/{self.C.log_file} only input.txt", "a", encoding="utf-8") as log_file:
